...
const tabsListVariants = variants({
  slots: {
    ...
    trigger: cn(
      ...
      // A finger's 40px target, centred on the tab and spilling into the row above and below
      // rather than growing the tab: a `min-height` here would stretch the pill, and with it
      // the card header the tabs sit in — which is what it did on iOS, where `pointer: coarse`
      // actually matches and a desktop browser's mobile emulation doesn't.
      "pointer-coarse:before:absolute pointer-coarse:before:inset-x-0 pointer-coarse:before:top-1/2 pointer-coarse:before:h-10 pointer-coarse:before:-translate-y-1/2",
    ),
  },
